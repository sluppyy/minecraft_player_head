...
module.exports.getUserId = async function(username) {
  ...
  return (await axios.get(`https://api.mojang.com/users/profiles/minecraft/${username}`)).data.id
}
...
module.exports.getSkinUrl = async function(uuid) {
  ...
  const string = (await axios.get(`https://sessionserver.mojang.com/session/minecraft/profile/${uuid}`)).data.properties[0].value
